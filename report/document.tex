...
\begin{document}
	...
	Κατά τη φάση σχεδιασμού εξετάστηκαν δύο βασικές προσεγγίσεις για την ενημέρωση της κατάστασης των παραγγελιών:
	\begin{itemize}
		...
		\item \textbf{Server-Sent Events (SSE):} Πρόκειται για ένα πρότυπο του HTML5 που επιτρέπει στον διακομιστή να στέλνει δεδομένα προς τον πελάτη μέσω μιας απλής, διαρκούς σύνδεσης HTTP. Προσφέρει χαμηλό overhead, αυτόματη επανασύνδεση από το πρόγραμμα περιήγησης και είναι ιδανικό για μονοκατευθυντική ροή δεδομένων πραγματικού χρόνου.
	\end{itemize}
...
const source = new EventSource('/api/notifications/stream');
	\end{verbatim}
	Όταν λαμβάνεται ένα νέο μήνυμα, ο client-side κώδικας αναλύει τα δεδομένα. Εάν εντοπιστεί αλλαγή στην κατάσταση κάποιας παραγγελίας, εμφανίζεται μια ενημέρωση στην οθόνη και πραγματοποιείται ασύγχρονη μερική ανανέωση του DOM της σελίδας παρακολούθησης.
